    ...
    switch (element.name) {
      ...
      default:
        element.oninput = () => {
          ...
          element.nextElementSibling.innerText = element.value.toString();
          updateSurface();
        };
    }
...
{
  const element = document.querySelector(`select#sliceAxis`);
...

{
  ...
  for (let i = 0; i < XYZ.length; i++) {
    ...
    element.value = rData[ch.toLowerCase()].toString();
...
const boxMaterial = new THREE.MeshPhongMaterial({color: 0xffffff,shininess: 80,side: THREE.FrontSide,vertexColors: true});


let boxGeometry = blockGeometry(
  (x,y) => {
  return rData.z.evaluate({x,y});
},
rData.a.evaluate(),
rData.b.evaluate(),
rData.c.evaluate(),
