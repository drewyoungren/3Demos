...
function updateTrails(F, dt = 0.016) {
  ...
  if (
    norm1(pos1) > point.lim ||
    (dt > 1e-6 && pos1.clone().sub(point.position).length() < 1e-3)
  ) {
    point.position.copy(
      point.start
        .clone()
  ...
  point.geometry =
    arrowGeometries[Math.max(0, Math.min(arrowGeometries.length - 1, height))];
...
function ruFrame({ u = 0.5, v = 0.5, dt = 0.001, du = 1, dv = 1 } = {}) {
  ...
  const p = new THREE.Vector3(
    ...
    ruForward = new THREE.Vector3(
      x.evaluate({ x: U + dt / 2, y: V }),
      y.evaluate({ x: U + dt / 2, y: V }),
      z.evaluate({ x: U + dt / 2, y: V })
    ),
    ...
    rvForward = new THREE.Vector3(
      x.evaluate({ x: U, y: V + dt / 2 }),
      y.evaluate({ x: U, y: V + dt / 2 }),
      z.evaluate({ x: U, y: V + dt / 2 })
    ),
...
function tangentVectors({ u = 0.5, v = 0.5, dt = 0.001, plane = true } = {}) {
  ...
  const tangentPlaneGeometry = new THREE.ParametricBufferGeometry(
    (u, v, vec) => {
      ...
      vec
        .copy(dr.p)
        .add(dr.u.clone().multiplyScalar(U))
        .add(dr.v.clone().multiplyScalar(V));
