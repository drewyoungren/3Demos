  ...
  element.onclick = () => {
    ...
    for (let j = 0; j < curveKeys.length; j++) {
      ...
      const elForm = document.querySelector(
        `.surface-choices-item#${curveKeys[j]}-formula`
      );
      if (i === j) {
...
function tnbFrame({ u = 0.5, dt = 0.001, du = 1, normalized = false } = {}) {
  const { a, b, x, y, z } = rData;
  const A = a.evaluate(),
    B = b.evaluate();
  const U = (1 - u) * A + u * B;
  ...
  const p = new THREE.Vector3(
    ...
    rForward = new THREE.Vector3(
      x.evaluate({ t: U + dt / 2 }),
      y.evaluate({ t: U + dt / 2 }),
      z.evaluate({ t: U + dt / 2 })
    ),
    rBackward = new THREE.Vector3(
      x.evaluate({ t: U - dt / 2 }),
      y.evaluate({ t: U - dt / 2 }),
      z.evaluate({ t: U - dt / 2 })
    );
  ...
  return {
    ...
    u: rForward,
    v: acc,
    n: rForward.clone().cross(acc).normalize(),
  };
}

// Construct tangent vector at a point u (both 0 to 1)
function tangentVectors({ u = 0.5, dt = 0.001 } = {}) {
