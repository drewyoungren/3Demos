...
const xmin = 3; // domain of function
const xmax = 4;
const ymin = -1;
const ymax = 3;
...
let scenes = [], canvases = [], renderers = [];

for (let i = 0; i < 2; i++) {
  const scene = new THREE.Scene();
  scenes.push(scene);
  console.log("#c" + i.toString())
  const canvas = document.querySelector("#c" + (i + 1).toString());
...
scenes[0].background = new THREE.Color( 0xddddef );

const camera = new THREE.PerspectiveCamera(75, canvases[0].clientWidth/canvases[0].clientHeight, 0.1, 1000);
...
camera.position.z = 2*gridMax;
camera.position.y = 1*gridMax ;
camera.position.x = 1.5*gridMax;
camera.lookAt( 0,0,0 );
...
scenes.forEach((scene) => {scene.add(gridMeshes);} );
drawGrid('rect');
...
function render() {
    ...
    if (resizeRendererToDisplaySize(renderer)) {
        ...
        console.log("cam aspect",camera.aspect);
